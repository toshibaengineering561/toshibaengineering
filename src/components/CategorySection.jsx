    ...
    <section className="section bg-neutral-50/60 dark:bg-neutral-900/30">
      <div className="container">
        <div className="flex items-end justify-between mb-6">
          ...
          <Link to="/products" className="text-sm text-brand-700 dark:text-brand-400">
            View all products
          </Link>
        </div>
              ...
              <div className="mt-3 grid grid-cols-3 gap-2">
                {getFeatured(c.id).map((p) => (
                  <Link
                    to={`/products/${p.id}`}
                    key={p.id}
                    className="group rounded border border-neutral-200 dark:border-neutral-800 overflow-hidden bg-white dark:bg-neutral-900"
                  >
                    <img
                      src={p.image}
                      alt={p.name}
